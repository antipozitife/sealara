...
import { detectRegionByPhone, isFrontendDemo, login, register } from "../../lib/auth-api";
import "../../styles/layout-shell.css";
import "./auth.css";

export const AuthPage: React.FC = () => {
  const [mode, setMode] = useState<"login" | "register">("login");
  const [surname, setSurname] = useState(isFrontendDemo ? "Иванова" : "");
  const [name, setName] = useState(isFrontendDemo ? "Анна" : "");
  const [patronymic, setPatronymic] = useState(isFrontendDemo ? "Сергеевна" : "");
  const [birthDate, setBirthDate] = useState(isFrontendDemo ? "1995-01-01" : "");
  const [gender, setGender] = useState<"м" | "ж" | "">(isFrontendDemo ? "ж" : "");
